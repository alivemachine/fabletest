...
- **M4  LOD + Resolver.** *(started — the stateful `EcoSim` "Vitality" layer.)* This is the first thing in the engine that is **not** a pure function of `t`: a coarse ecosystem (fertility, vegetation, fauna, civilisation, burn/salt scars) that integrates forward with MEMORY, so floods/droughts/fires leave lasting scars and recovery is slow and neighbour-seeded — worlds can fail to come back. It is deliberately not seekable (only forward + reset), quarantined to one layer so the seekable core is untouched. It is the far-form stock field every near-form agent will expand out of / collapse back into. Still to come: the *Resolver* proper (expand/collapse + budget cap), the *society* expansion (a coarse cell's faction/population/stress → a deterministic settlement grammar keyed by the cell's seed), and near-form herds (boids) / NPCs once zoom reaches their scale. Agents = people + animals + storms, one code path.
